...
static void COFFEE(double *local_sinkhorn, double *local_sumofcol, double *sumofcol, const int m, const int n, const int world_rank)
{
        double time2, time02;
        double start = MPI_Wtime();
        __m256d vec_0 = _mm256_setzero_pd();
        __m256d vec_1 = _mm256_setzero_pd();
        __m256d vec_2 = _mm256_setzero_pd();
        __m256d vec_3 = _mm256_setzero_pd();
        __m256d vec_t = _mm256_setzero_pd();
        ...
        for (int i = 0; i < m * n / 8; i++, p1 += 8)
        {
                ...
                vec_0 = _mm256_add_pd(vec_0, vec_2);
                _mm256_storeu_pd(p0, vec_0);
                vec_1 = _mm256_add_pd(vec_1, vec_3);
                _mm256_storeu_pd(p0 + 4, vec_1);
                tep1 += 8;
                if (tep1 == n)
                {
                        tep1 = 0;
                ...
                for (int j = 0; j < kk; j++)
                {
                        for (int r = 0; r < rr; r++)
                        {
                                for (int t = 0; t < ii; t += 4, z0 += 4, z1 += 4, z2 += 4, z3 += 4)
                                {
                                        ...
                                        vec_0 = _mm256_loadu_pd(z0);
                                        vec_0 = _mm256_mul_pd(vec_0, vec_t);
                                        _mm256_storeu_pd(z0, vec_0);
                                        vec_1 = _mm256_loadu_pd(z1);
                                        vec_1 = _mm256_mul_pd(vec_1, vec_t);
                                        _mm256_storeu_pd(z1, vec_1);
                                        vec_2 = _mm256_loadu_pd(z2);
                                        vec_2 = _mm256_mul_pd(vec_2, vec_t);
                                        _mm256_storeu_pd(z2, vec_2);
                                        vec_3 = _mm256_loadu_pd(z3);
                                        vec_3 = _mm256_mul_pd(vec_3, vec_t);
                                        _mm256_storeu_pd(z3, vec_3);
                                }
